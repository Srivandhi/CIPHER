...
# base directory for logs and other static files
BASE_DIR = Path(__file__).parent.parent
LOG_DIR = BASE_DIR  # currently same, adjust if needed
...
async def validation_exception_handler(request, exc):
    ...
    log_file = LOG_DIR / "debug_val_error.log"
...
def get_atm_hotspots(complaint: Complaint, db: Session = Depends(get_db)):
    ...
    try:
        ...
        import traceback
        log_file = LOG_DIR / "debug_err.log"
        with open(log_file, "a") as f:
